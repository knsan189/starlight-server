...
export default class FortuneService {
  public static async getFortunes(page: number, size: number): Promise<Fortune[]> {
  ...
  public static async getCount(): Promise<number> {
    return new Promise((resolve, reject) => {
      (async () => {
        getConnection((connection: PoolConnection) => {
          try {
            connection.query("SELECT COUNT(*) AS count FROM Fortune", (err, result) => {
              if (err) throw new Error(err.message);
  ...
  public static async editFortune(id: number, fortune: Fortune): Promise<string> {
    return new Promise((resolve, reject) => {
      (async () => {
        getConnection((connection: PoolConnection) => {
          try {
            const sql = "UPDATE Fortune SET fortune=?, msg=?, delayTime=? WHERE id=?";
            const values = [fortune.fortune, fortune.msg, fortune.delayTime, id];
